...
public abstract class CooperativeMiniGame extends MiniGame {
	...
	private void addPlayerToWaitList(Player p) {
		...
		if (this.activated == true) {
			BroadcastTool.sendMessage(p, "미니게임이 이미 플레이 중입니다");
		}
		...
		if (!this.waitPlayers.contains(p)) {
			...
			// master에게 알리기
			BroadcastTool.sendMessage(this.master, p.getName() + " 님이 미니게임에 참가하고 싶어합니다");
			BroadcastTool.sendMessage(this.master, "/re minigame ok <닉네임>으로 참여");
			BroadcastTool.sendMessage(this.master, "/re minigame kick <닉네임>으로 강퇴");
			// waiter에게 알리기
			BroadcastTool.sendMessage(p, "접수됬습니다. 미니게임 방장의 허락을 기다리세요");
		}
	}
	...
	public void okWaitingPlayer(Player waiter, PlayerData waiterPData) {
		...
		if (this.checkPlayerCountFull()) {
			BroadcastTool.sendMessage(this.getMaster(), this.gameType.name() + " 미니게임 플레이어 인원수가 꽉 찾습니다");
			BroadcastTool.sendMessage(waiter, this.gameType.name() + " 미니게임 플레이어 인원수가 꽉 찾습니다");
			return;
		}

		// wait list에 있는 플레이어인지 체크
		if (!this.waitPlayers.contains(waiter)) {
			BroadcastTool.sendMessage(this.getMaster(), waiter + " 님은 신청하지 않은 사람입니다");
			return;
		}

		// token충분한지 검사
		if (!waiterPData.minusToken(this.getFee())) {
			BroadcastTool.sendMessage(waiter, this.gameType + " 미니게임에 참여할 토큰이 부족합니다");
			BroadcastTool.sendMessage(this.master, waiter + "님이 참여할 토큰이 부족합니다");
			return;
		}
		...
		// 허락 메세지
		BroadcastTool.sendMessage(this.master, waiter.getName() + "님이 " + this.gameType.name() + " 미니게임에 참여했습니다");
		BroadcastTool.sendMessage(waiter, this.master.getName() + " 미니게임에 참여했습니다");
	}
	...
	public void kickPlayer(Player p) {
		...
		// info
		BroadcastTool.sendMessage(p, "당신은 " + this.gameType.name() + " 미니게임에서 추방되었습니다");
	}
	...
	private void printGameResult() {
		...
		BroadcastTool.sendMessage(this.players, "" + ChatColor.RED + ChatColor.BOLD + "게임 종료");
		BroadcastTool.sendMessage(this.players, "=================================");

		// score 공개
		BroadcastTool.sendMessage(this.players, "팀 점수" + this.score);

		// send title
		BroadcastTool.sendTitle(this.players, "게임 종료", "");
		BroadcastTool.sendMessage(this.players, "");
	}
...
//				BroadcastTool.sendMessage(all, "Your team is in " + quartileIndex + " quartile");
				BroadcastTool.sendMessage(all, "팀 점수는 " + topPercent + " 입니다");
				BroadcastTool.sendMessage(all, "보상 토큰: " + rewardToken);

				pData.plusToken(rewardToken);
			} else if (isFirstScore) {
				// 1,2,3,4 분위 안에 속해있지 않다는것 = 1등 점수
				BroadcastTool.sendMessage(all, "팀이 1등했습니다");
				BroadcastTool.sendMessage(all, "보상 토큰: " + this.getFee() * 3);
