		...
		if (event instanceof EntityDamageByEntityEvent) {
			...
			if (e.getEntity() instanceof Player && e.getDamager() instanceof Player) {
				...
				if (!this.checkHitPlayer(damager)) {
					BroadcastTool.sendMessage(damager, "다른 플레이어 턴 차례입니다");
					return;
				}
				...
				if (victimIsDead) {
					...
					BroadcastTool.sendMessage(victim, "게임이 끝날때까지 기다리세요");
					// damager
...
//	}
	...
	private void startHitTimer() {
		...
		this.hitTask = Bukkit.getScheduler().runTaskTimer(Main.getInstance(), new Runnable() {
			...
			public void run() {
				hitTimeLeft -= 1;
				BroadcastTool.sendMessage(getHitPlayer(), "남은 때릴 시간: " + hitTimeLeft);
				if (hitTimeLeft <= 0) {
					BroadcastTool.sendMessage(getHitPlayer(), "때릴 수 있는 시간이 지났습니다!");
...
//    }
	...
	private void nextHitIndex() {
		...
		BroadcastTool.sendMessage(hitPlayer, "당신이 때릴 차례입니다");
	}
